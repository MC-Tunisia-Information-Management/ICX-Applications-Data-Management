...
/*function dataExtraction(query)
{
  var requestOptions = {
    ...
    'contentType':'application/json',
    'headers':{
      'access_token': "28d836e9795a820a636ef426887ebac5e7ac6a415e49336a7f0dff2c4015d8e2"
    }
  };
...
function dataUpdating(){
  ...
  var sheetApplications = SpreadsheetApp.getActiveSpreadsheet().getSheetByName("1Applications"); // write sheet name
//  var startDate = Utilities.formatDate(sheetInterface.getRange(7,2).getValue(), "GMT+2", "dd/MM/yyyy");
var startDate = "01/07/2023"
  var page_number = 1
  var allData = []
  do{
  ...
  for(let data of allData){
    for(let i = 0; i < data.length; i++){
      console.log(i)
       var duplicatedRowIndex = sheetApplications.createTextFinder(data[i].id).matchEntireCell(true).findAll().map(x => x.getRow())
      let duplicated = duplicatedRowIndex.length == 0? false: true
        if(!duplicated){      
          Logger.log(i)
          newRows.push([
          ...
          sheetApplications.getRange(duplicatedRowIndex[0],9).setValue(data[i].status)
        }
      }
    }
    if(newRows.length > 0){
        sheetApplications.getRange(sheetApplications.getLastRow()+1,1,newRows.length,newRows[0].length).setValues(newRows)
    }
  var now = new Date();
  sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName("Interface"); // write sheet name
  updateDate = sheet.getRange(7,4).setValue(now);
...
function onOpen() {  
  var ui = SpreadsheetApp.getUi();
  ui.createMenu('Run')
      .addItem("Run the code", 'dataUpdating')
      .addToUi();
}
